...
import PrintListView from './components/PrintListView.js';
...
const App = () => {
  ...
  const [printMode, setPrintMode] = useState('board');
  ...
  const handleRegisterBulk = async (payload) => {
    setLoading(true);
  ...
  const handleUpdateAthlete = async (id, updatedData) => {
    setLoading(true);
  ...
  const openPrintPreview = (data, mode) => {
